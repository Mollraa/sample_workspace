...
public class ex03 {
	public static void main(String[] args) {
		...
		System.out.print("x: ");
		int x = Integer.parseInt(scn.nextLine()); // 정수입력값을 받는다.
		System.out.print("y: ");
		int y = Integer.parseInt(scn.nextLine()); // 문자값을 int 값으로 받음
		if (x > 0) { // 이중 if문
...
// 한 단어를 입력받을 경우 Scanner객체의 next() 함수를 사용하면 됩니다.
		...
		while (run) { //반복문으로 run 실행 
			...
			String rps = scn.nextLine(); //문자타입으로 값을 받겠다~
			...
			case "끝": 
				...
				run = false; //런이 거짓일때 종료 
				break; //while문 종료
			default: //가위바위보 다 아닐 때 입력받은 값
