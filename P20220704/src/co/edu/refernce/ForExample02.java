...
public class ForExample02 { // 메소드 정의구문
...
public static void main(String[] args) { // 메소드 호출구문
// 배열 합계. int sum = 0;
double avg = 0;
int[] intAry = { 34, 23, 56, 22, 99, 28 };
sum = sumAry(intAry);
avg = (double) sum / intAry.length;
System.out.printf("intAry의 합은 %d, 평균은 %.1f\n", sum, avg); // intAry 배열의 합과 평균.
sum = 0;
...
System.out.printf("intArray 배열의 합 %d, 평균 %.1f\n", sum, avg); // intArray 배열의 합과 평균.
sum = 0;
int[] scanArray = new int[5];
Scanner scn = new Scanner(System.in);
for (int i = 0; i < scanArray.length; i++) { System.out.println("임의의 수를 입력 >>>>"); scanArray[i] = scn.nextInt();
